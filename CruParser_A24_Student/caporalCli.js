...
cli
  ...
  .option("-t, --showTokenize", "log thenode  tokenization results", {
    validator: cli.BOOLEAN,
    default: false,
  })
  ...
  .option("-n, --needle <needle>", "Search a needle in parsed lines", {
    validator: cli.STRING,
  })
  .option("-d, --day <day>", "Filter by day (L, MA, ME, J, V)", {
    validator: cli.STRING,
  })
  .action(({ args, options, logger }) => {
    fs.readFile(args.file, "utf8", function (err, data) {
  ...
  .command(
    "salleCours",
    "Output the classrooms associated to the class associated with <name>"
  )
  .argument("<name>", "The name of the class")
  // Only one arg, if 0 or 2+ will print an error automatically
  .option(
    "-c, --filtercm",
    "Filters only by the CM classes, use only one filter at a time",
    { validator: cli.BOOLEAN, default: false }
  )
  .option(
    "-d, --filtertd",
    "Filters only by the TD classes, use only one filter at a time",
    { validator: cli.BOOLEAN, default: false }
  )
  .option(
    "-t, --filtertp",
    "Filters only by the TP classes, use only one filter at a time",
    { validator: cli.BOOLEAN, default: false }
  )
  .action(({ args, options, logger }) => {
    ...
    try {
      ...
      elements.forEach((element) => {
        ...
        if (element.isFile()) {
          ...
          try {
            ...
            analyzer.parsedCru.forEach((Cru) => {
              ...
              if (options.c) {
                // Ajout de la vérification du type 'C' (CM)
                if (Cru.cours === args.name && Cru.type && Cru.type[0] === "C")
                  arraySalleCours.push(Cru.salle);
              }
              if (options.d) {
                // Ajout de la vérification du type 'D' (TD)
                if (Cru.cours === args.name && Cru.type && Cru.type[0] === "D")
                  arraySalleCours.push(Cru.salle);
              }
              if (options.t) {
                // Ajout de la vérification du type 'T' (TP)
                if (Cru.cours === args.name && Cru.type && Cru.type[0] === "T")
                  arraySalleCours.push(Cru.salle);
              }
            });
          } catch (err) {
        ...
        // If encounters a sub directory, will read the files inside them
          ...
          try {
            ...
            sub_files.forEach((file) => {
              if (file.isFile()) {
                ...
                try {
                  ...
                  logger.warn(
                    `Impossible de lire ${filepath} : ${err.message}`
                  );
                }
              }
            });
            ...
            logger.warn(
              `Impossible de lire le sous-dossier ${sub_dir} : ${err.message}`
            );
          }
        }
      });
      ...
      if (arraySalleCours.length === 0) {
        ...
        logger.info(
          "Voici la liste des salles associées au cours " +
            args.name +
            ":\n" +
            arraySalleCours
        );
      ...
      logger.error(
        `Impossible de lire le dossier ${data_dir} : ${err.message}`
      );
    }
  })
  ...
  .action(({ args, logger }) => {
    ...
    fs.readdir(rootFolder, { withFileTypes: true }, (err, files) => {
      ...
      const maxCap = Math.max(
        ...roomCourses.map((c) => parseInt(c.capacite, 10))
      );
        ...
        if (dirent.isDirectory()) {
          ...
          fs.readFile(filePath, "utf8", (err, data) => {
            ...
            if (filesToRead === 0) {
              ...
              // Nouvel objet formaté
              const formattedFreeSlots = {};
              
              // Transformation directe dans la boucle
              for (const day of days) {
                const hours = freeSlots[day];
                const ranges = [];
                ...
                for (let i = 0; i < hours.length; i++) {
                  ...
                  // Si la suite s'arrête ou fin du tableau
                  if (next !== current + 1) {
                    ranges.push(`${startHour}h-${current + 1}h`);
                    startHour = null;
                  }
                }
              
                formattedFreeSlots[dayNames[day]] = ranges;
              }
  ...
  .action(({ args, options, logger }) => {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      return logger.error(
        "Le format de date n'est pas valide. Utilisez ce format : YYYY-MM-DD"
      );
    }
  ...
  .option("-d, --detailed", "Show detailed conflict information", {
    validator: cli.BOOLEAN,
    default: false,
  })
  ...
  .action(({ args, logger }) => {
    ...
    if (!validDays.includes(day)) {
      return logger.error(
        "Invalid day  . Format should be : 'L' or 'MA' or 'ME' or 'J' or 'V'"
      );
    }
    if (start > end) {
    ...
    if (endMin - startMin < 60) {
      return logger.error(
        "Invalid time range, the time slot should be at least one hour long. "
      );
    }
    ...
    fs.readdir(rootFolder, { withFileTypes: true }, (err, files) => {
      ...
      const occupiedClasses = coursesOfDay
        .filter(
          (c) => toMin(c.heureDeb) < endMin && toMin(c.heureFin) > startMin
        )
        .map((c) => c.salle);
      const occupiedClassesunique = [...new Set(occupiedClasses)];
      const allclasses = [...new Set(allCourses.map((c) => c.salle))];
      const freeclasses = allclasses.filter(
        (room) => !occupiedClassesunique.includes(room)
      );
  ...
  .command(
    "tauxSalles",
    "Gives a graphical representation of classroom occupation rate in a time frame"
  )
  ...
  .argument(
    "<firstDay>",
    'Starting day of the time period : "L", "MA", "ME", "J", "V"'
  )
  .argument(
    "<firstHour>",
    "Starting hour of the time period, from 8:00 to 19:00 : examples -> 8:00, 10:30"
  )
  .argument("<firstWeek>", 'Starting week of the time period : "F0" to "F9"')
  .argument(
    "<lastDay>",
    'Ending day of the time period : "L", "MA", "ME", "J", "V"'
  )
  .argument(
    "<lastHour>",
    "Ending hour of the time period, from 9:00 to 20:00 : examples -> 9:30, 20:00"
  )
  ...
  .option(
    "-e, --export",
    "Exports the rooms with their associated rates in a .csv file",
    { validator: cli.BOOLEAN, default: false }
  )
  .action(({ args, options, logger }) => {
    ...
    const hoursLastDayToReference = convertHoursToReference(
      parseInt(args.lastWeek.substring(1)),
      arrDays.indexOf(args.lastDay),
      parseInt(args.lastHour.split(":")[0]),
      parseInt(args.lastHour.split(":")[1])
    );
    const hoursFirstDayToReference = convertHoursToReference(
      parseInt(args.firstWeek.substring(1)),
      arrDays.indexOf(args.firstDay),
      parseInt(args.firstHour.split(":")[0]),
      parseInt(args.firstHour.split(":")[1])
    );
      ...
      elements.forEach((element) => {
        ...
        if (element.isFile()) {
          ...
          try {
            ...
            analyzer.parsedCru.forEach((Cru) => {
              const classStartToReference = convertHoursToReference(
                parseInt(Cru.semaine),
                arrDays.indexOf(Cru.jour),
                parseInt(Cru.heureDeb.split(":")[0]),
                parseInt(Cru.heureDeb.split(":")[1])
              );

              const classEndToReference = convertHoursToReference(
                parseInt(Cru.semaine),
                arrDays.indexOf(Cru.jour),
                parseInt(Cru.heureFin.split(":")[0]),
                parseInt(Cru.heureFin.split(":")[1])
              );
              // Check filters

              if (!options.c && !options.d && !options.t) {
                if (
                  classEndToReference >= hoursFirstDayToReference &&
                  classStartToReference <= hoursLastDayToReference
                ) {
    ...
    arrayGroupBySalle.forEach((group) => {
      ...
      group.forEach((cours) => {
        ...
        const hourClassStart = convertHoursToReference(
          parseInt(cours.semaine),
          arrDays.indexOf(cours.jour),
          parseInt(cours.heureDeb.split(":")[0]),
          parseInt(cours.heureDeb.split(":")[1])
        );
        const hourClassEnd = convertHoursToReference(
          parseInt(cours.semaine),
          arrDays.indexOf(cours.jour),
          parseInt(cours.heureFin.split(":")[0]),
          parseInt(cours.heureFin.split(":")[1])
        );
    ...
    async function printGraph() {
      ...
      const data = Object.entries(json_tauxSalles).map(([Salle, Rate]) => ({
        Salle,
        Rate,
      }));
      const vlSpec = {
...
function checkSchedulingConflicts(allCourses, detailed, logger) {
  ...
  allCourses.forEach((course) => {
    if (!course.salle || !course.jour || !course.heureDeb || !course.heureFin)
      return;
  ...
  if (conflicts.length === 0) {
    logger.info(
      "✓ Aucun conflit détecté ! Tous les créneaux sont cohérents.".green
    );
  } else {
    ...
    conflicts.forEach((conflict, index) => {
      ...
      logger.info(
        `  Cours 1: ${conflict.course1.name} ${conflict.course1.type} (${
          conflict.course1.horaire
        })${
          conflict.course1.semaine
            ? " - Semaine " + conflict.course1.semaine
            : ""
        }`
      );
      logger.info(
        `  Cours 2: ${conflict.course2.name} ${conflict.course2.type} (${
          conflict.course2.horaire
        })${
          conflict.course2.semaine
            ? " - Semaine " + conflict.course2.semaine
            : ""
        }`
      );
...
function generateICalendar(
  ...
  if (filteredCourses.length === 0) {
    return logger.error(
      "Aucun cours correspondant : " + requestedCourses.join(", ")
    );
  }
  ...
  filteredCourses.forEach((course) => {
    ...
    while (currentDate <= endDate) {
      if (currentDate.getDay() === targetDay) {
        ...
        const uid = `${course.cours}-${course.index}-${formatDate(
          eventStart
        )}@cruparser`;
        const summary = `${course.cours} - ${course.type}`;
        const description = `Cours: ${course.cours}\\nType: ${
          course.type
        }\\nCapacité: ${course.capacite}\\nHoraire: ${course.heureDeb.replace(
          ":",
          "h"
        )} - ${course.heureFin.replace(":", "h")}\\nSemaine: ${
          course.semaine || "N/A"
        }`;
  ...
  fs.writeFile(outputFile, icalContent.join("\r\n"), "utf8", (err) => {
    ...
    logger.info(
      ...
      } course entries for ${requestedCourses.join(", ")}`
    );
  });
}
