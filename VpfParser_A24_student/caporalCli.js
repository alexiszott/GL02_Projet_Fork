...
cli
    ...
    .action(({args, options, logger}) => {
        fs.readFile(args.file, 'utf8', function (err,data) {
            ...
            if(analyzer.errorCount === 0){
                ...
                // console.log(poiAFiltrer);
                let resultat = poiAFiltrer.filter((poi) => poi.name.toLowerCase().includes(args.needle.toLowerCase()));
                logger.info("%s", JSON.stringify(resultat, null, 2));
                // Filtre à ajouter //
    ...
    .action(({args, options, logger}) => {
        fs.readFile(args.file, 'utf8', function (err,data) {
            ...
            analyzer.parse(data);
            if (analyzer.errorCount === 0){
                let poiAvecMoyenne = analyzer.parsedPOI.map((poi) => {
                    return {
                        name: poi.name,
                        lat: poi.lat,
                        lng: poi.lng,
                        ratings: poi.ratings,
                        averageRatings: poi.ratings.length > 0 ? poi.averageRatings() : 0
                    };
                });
                logger.info("%s", JSON.stringify(poiAvecMoyenne, null, 2));
            }else{
                logger.info("The .vpf file contains error".red);
            };
        });
    ...
    // abc
    .command('abc', 'restructuring the POI data by the first letter of their name')
    .argument('<file>', 'The Vpf file to use')
    .action(({args, options, logger}) => {
        fs.readFile(args.file, 'utf8', function (err,data) {
            if (err) {
            ...
            if(analyzer.errorCount === 0){
                let poiParLettre = analyzer.parsedPOI.reduce((acc, poi) => {
                    let firstLetter = poi.name.charAt(0).toUpperCase();
                    if (!acc[firstLetter]) {
                        acc[firstLetter] = [];
                    }
                ...
                logger.info("%s", JSON.stringify(poiParLettre, null, 2));
            }else{
                logger.info("The .vpf file contains error".red);
            }
        });
    ...
    .action(({args, options, logger}) => {
        fs.readFile(args.file, 'utf8', function (err,data) {
            ...
            if(analyzer.errorCount === 0){
                ...
                let avg = analyzer.parsedPOI.map((poi) => {
                    return {
                        name: poi.name,
                        averageRatings: poi.ratings.length > 0 ? poi.averageRatings() : 0
                    };
